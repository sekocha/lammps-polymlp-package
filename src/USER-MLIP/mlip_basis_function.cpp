...
void bump_cutoff_function_d(const double& dis, const double& cutoff,
...
void gauss_d
(const double& dis, const double& p1, const double& p2,
 double& bf, double& bf_d){
    bf = gauss(dis, p1, p2);
...
void cosine_d
(const double& dis, const double& p1, double& bf, double& bf_d){
    bf = cosine(dis, p1);
    bf_d = - p1 * sin(p1 * dis);
}
...
double sph_bessel(const double& dis, const double& p1){
    return boost::math::sph_bessel(round(p1), dis);
}
...
void sto_d
(const double& dis, const double& param1, const double& param2, 
 double& bf, double& bf_d){
...
//else if (basis == "erf") descriptor = erf(param[0] * dis) / dis;
//else if (basis == "erfc") descriptor = erfc(param[0] * dis) / dis;
//else if (basis == "mbessel1")
//    descriptor = boost::math::cyl_bessel_i(round(param[0]), dis);
//else if (basis == "mbessel2")
//    descriptor = boost::math::cyl_bessel_k(round(param[0]), dis);
//else if (basis == "legendre")
//    descriptor = boost::math::legendre_p(round(param[0]), dis);
//else if (basis == "hermite")
//    descriptor = boost::math::hermite(round(param[0]), dis);
//else if (basis == "laguerre")
//    descriptor = boost::math::laguerre(round(param[0]), dis);
/*        
          else if (basis == "erf"){ 
          descriptor = erf(param[0] * dis) / dis;
          }
          else if (basis == "erfc"){
          descriptor = erfc(param[0] * dis) / dis;
          }
          else if (basis == "legendre"){
          descriptor = boost::math::legendre_p(round(param[0]), dis);
          prod1 =  dis * descriptor 
          - boost::math::legendre_p(round(param[0]) + 1, dis);
          descriptor_diff = (param[0] + 1.0) * prod1 / (1 - dis * dis);
          }
          else if (basis == "hermite"){
          descriptor = boost::math::hermite(round(param[0]), dis);
          descriptor_diff = 2.0 * dis * descriptor 
          - boost::math::hermite(round(param[0]) + 1, dis);
          }
          else if (basis == "laguerre"){
          descriptor = boost::math::laguerre(round(param[0]), dis);
          if (round(param[0]) == 0)   descriptor_diff = 0.0;
          else {
          prod1 = descriptor 
          - boost::math::laguerre(round(param[0])-1, dis);
          descriptor_diff = param[0] * prod1 / dis;
          }
          }
 */
